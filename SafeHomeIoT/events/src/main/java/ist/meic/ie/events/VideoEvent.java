...
public class VideoEvent extends Event{
    ...
    public void checkDB(DatabaseConnect config, String userDB){
        try {
            ...
            create_db.execute();
            create_db.close();
            PreparedStatement create_table = config.getConnection().prepareStatement("CREATE TABLE IF NOT EXISTS "+userDB+".videoMessage (ID INT AUTO_INCREMENT,deviceID INT, description VARCHAR(30), type VARCHAR(30) ,ts TIMESTAMP DEFAULT CURRENT_TIMESTAMP, userID INT, PRIMARY KEY(ID));");
